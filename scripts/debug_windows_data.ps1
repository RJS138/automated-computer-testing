...
if ($fc) {
    ...
    $fc | Format-List | Out-String | ForEach-Object { Write-Host "  $_" }
} else {
...
if ($cc) {
    ...
    Write-Host ""
    Write-Host "  All properties:"
    $cc | Format-List | Out-String | ForEach-Object { Write-Host "  $_" }
} else {
...
# ------------------------------------------------------------------------------
Show-Section "11. Full JSON payload (what the app sends to Python)"
# ------------------------------------------------------------------------------
...
$wmicOut = & wmic bios get Manufacturer /format:list 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Host "  wmic is available. Sample output:" -ForegroundColor Green
    $wmicOut | Where-Object { $_ -match '\S' } | ForEach-Object { Write-Host "    $_" }
} else {
    Write-Host "  wmic returned exit code $LASTEXITCODE" -ForegroundColor Yellow
    Write-Host "  (deprecated in Windows 11 24H2+ but usually still present)" -ForegroundColor Yellow
}

Write-Host ""
Write-Host $sep -ForegroundColor Green
Write-Host "  Diagnostic complete -- paste output above back to diagnose." -ForegroundColor Green
Write-Host $sep -ForegroundColor Green
Write-Host ""
