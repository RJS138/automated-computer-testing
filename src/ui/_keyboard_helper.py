...
def run_keyboard_test() -> bool:
    ...
    _default_id = {"Darwin": "macbook_us", "Windows": "full_us"}.get(platform.system(), "tkl_us")
    ...
    tk.Label(layout_bar, text="Layout:", bg=_BG, fg="#888", font=("Courier", 11)).pack(
        side="left", padx=(0, 8)
    )
    ...
    tk.Label(layout_bar, text=" ▾", bg=_BG, fg="#666", font=("Courier", 11)).pack(side="left")
    ...
    for lid in layouts:
        layout_menu.add_command(
            ...
            command=lambda lid_=lid: _switch_layout(lid_),
        )
    ...
    _make_btn(btn_row, "Fail", "#8b1a1a", "white", _do_fail, "#a02020").pack(side="left", padx=10)
    ...
    _make_btn(btn_row, "Skip", "#3a3a3a", "#aaa", _do_skip, "#4a4a4a").pack(side="left", padx=10)
    ...
    def _draw():
        ...
        def row_units(row: _Row) -> float:
            return sum(item if isinstance(item, float) else item.width for item in row.items)
            ...
            for item in row.items:
                if isinstance(item, float):
                    ...
                    canvas.create_rectangle(
                        x, y, x + kw, y + kh, fill=fill, outline=outline, width=1
                    )
                    canvas.create_text(
                        x + kw / 2,
                        y + kh / 2,
