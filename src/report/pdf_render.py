...
_S = {
    ...
    "meta":     _style("meta",   fontName="Helvetica",      fontSize=8,  leading=11, textColor=HexColor("#cccccc")),
    ...
    "data_key": _style("dkey",   fontName="Helvetica-Bold", fontSize=8,  leading=10, textColor=HexColor("#444444")),
...
def _rows_cpu(d: dict) -> list[tuple[str, str]]:
    r = [
        ...
        ("Cores / Threads", f"{d.get('physical_cores','—')} / {d.get('logical_cores','—')}"),
...
def _rows_storage(d: dict) -> list[tuple[str, str]]:
    ...
    for i, drv in enumerate(d.get("drives", []), 1):
        ...
        cap_str = (f"{cap} GB total / {used or '—'} GB used / {free or '—'} GB free"
                   if cap else "—")
        rows += [
            (f"Drive {i} — {drv.get('model','Unknown')}", ""),
            ("  Serial / Interface", f"{drv.get('serial','—')} / {drv.get('interface','—')} {drv.get('medium_type','')}"),
            ("  Capacity", cap_str),
        ...
        if drv.get("percentage_used") is not None:
            rows.append(("  NVMe Wear / Spare", f"{drv['percentage_used']}% used / {drv.get('available_spare_pct','—')}% spare"))
    rows += [
...
def _rows_network(d: dict) -> list[tuple[str, str]]:
    ...
    rows.append(("Internet (ping)", f"{rtt} ms" if d.get("ping_reachable") and rtt else "Failed"))
    ...
    if nets:
        rows.append((f"Networks Visible ({len(nets)})",
...
def _rows_battery(d: dict) -> list[tuple[str, str]]:
    ...
    rows.append(("Cycle Count", _fmt(d.get("cycle_count")) if d.get("cycle_count") is not None else "Unknown"))
...
def _status_colors(status: str) -> tuple:
    ...
    s = status.lower()
    if s == "pass":   return _C["pass"], _C["pass_bg"], _C["pass_txt"]
    if s == "warn":   return _C["warn"], _C["warn_bg"], _C["warn_txt"]
    if s == "fail":   return _C["fail"], _C["fail_bg"], _C["fail_txt"]
...
def _header_band(job: dict, report_type: str, generated_at: str) -> Table:
    left = [
        ...
        Paragraph(f"Generated: {generated_at}  ·  Mode: {job.get('test_mode','').upper()}", _S["meta"]),
    ]
    right = Paragraph(
        f"<b>{report_type.upper()} REPAIR</b>",
        _style("rt", fontName="Helvetica-Bold", fontSize=10, textColor=white,
               alignment=2),
    )
...
def _summary_band(overall: str, pass_c: int, warn_c: int, fail_c: int) -> Table:
    border, bg, txt = _status_colors(overall)
...
def _test_result_block(result: dict) -> list:
    ...
    badge = Paragraph(f"<b>{status.upper()}</b>",
                      _style(f"b{status}", fontName="Helvetica-Bold", fontSize=7,
                             textColor=txt))
    header_table = Table(
        [[badge,
    ...
    if data:
        ...
        if rows:
            tdata = [[Paragraph(k, _S["data_key"]), Paragraph(_fmt(v) if not isinstance(v, str) else v, _S["data_val"])]
                     for k, v in rows]
...
def _build_comparison_pdf(data: dict, output_path: Path) -> bool:
    ...
    header_t = Table(
        [[Paragraph("Before / After Comparison Report", _S["h1"]),
          Paragraph(
              ...
              _style("chm", fontName="Helvetica", fontSize=9, textColor=HexColor("#cccccc"),
                     alignment=2),
          )]],
    ...
    thead = Table(
        [[Paragraph("<b>Test</b>", _style("th", fontName="Helvetica-Bold", fontSize=8, textColor=white)),
          Paragraph("<b>Before</b>", _style("th2", fontName="Helvetica-Bold", fontSize=8, textColor=white)),
          Paragraph("→", _style("arr", fontName="Helvetica-Bold", fontSize=10, textColor=white, alignment=1)),
          Paragraph("<b>After</b>", _style("th3", fontName="Helvetica-Bold", fontSize=8, textColor=white))]],
        colWidths=col_w,
    )
    ...
    _STATUS_HEX = {
        "pass": "#2e7d32", "warn": "#f57f17", "fail": "#c62828",
        "skip": "#757575", "error": "#c62828", "waiting": "#757575",
    }
    _ARROWS = {"improved": "^", "worsened": "v", "changed": "~", "unchanged": "-"}
    _ARROW_COLORS = {
        "improved": _C["pass_txt"], "worsened": _C["fail_txt"],
        "changed": _C["warn_txt"], "unchanged": _C["muted"],
    }
    ...
    for i, row in enumerate(rows):
        ...
        _, b_bg, _ = _status_colors(b_stat)
        _, a_bg, _ = _status_colors(a_stat)
        ...
        arrow_col = Paragraph(
            _ARROWS.get(change, "-"),
            _style(f"arr{i}", fontName="Helvetica-Bold", fontSize=12,
                   textColor=_ARROW_COLORS.get(change, _C["muted"]), alignment=1),
        )
        tbody_data.append([
            Paragraph(f"<b>{row.get('display_name', row.get('name',''))}</b>", _S["data_key"]),
            Paragraph(
